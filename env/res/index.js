...
var plugin = require("./package/index.js");
var $ = require('jquery');
setTimeout(function () {
...
function getPluginMessageInfo(message) {
    ...
    var match = content.trim().match(/^([a-zA-Z0-9_-]+)\s*\(([\s\S]*)\)\s*;?\s*$/);
    ...
    var typeInfo = plugin.messageList[name];
    if (!typeInfo) {
        return;
    }
    var showBase = typeInfo.showBase,
        process = typeInfo.process;
...
function findUserMessage(userName) {
    var fullMatch = false;
    var match = userName.match(/^"([\s\S]*)"$/);
    if (match) {
        userName = match[1];
        fullMatch = true;
    }
    var $names = $('.message-list-item').find('.nickname');
    var $item = void 0;
    for (var i = $names.length - 1; i >= 0; i--) {
        var thisName = $names.eq(i).text();
        if (fullMatch) {
...
exports.default = {
    ...
    getPluginMessageInfo: getPluginMessageInfo,
    getMessage: plugin.getMessage,
    findUserMessage: findUserMessage,
    timestamp: 0
};
},{"./package/index.js":5,"jquery":7}],2:[function(require,module,exports){
'use strict';
    ...
    setTimeout(function () {
        ...
        if ($source) {
            if ($source.direction === 'left') {
                ...
                $bomb.css('left', width / 2 + 'px').css('bottom', '-' + width / 2 + 'px').css('transform', 'translate(-50%,-50%)');
            }
        } else {}
        ...
        $bomb.css('opacity', '0').css('position', 'relative').animate({
            ...
            step: function step(now, fx) {
                ...
                $(this).css('transform', '').css('borderSpacing', '1000').css('left', pos1.left - pos2.left - width / 2).css('top', pos1.top - pos2.top + width / 2).css('transform', 'translate(50%,-50%) rotate(' + finalDeg + 'deg)');
            }
        });
    });
    $bomb.css('opacity', '0');
...
},{"./commands/boom.js":2,"./commands/shit.js":3,"./commands/system.js":4,"jquery":7,"jquery-image-explode":6,"jquery.easing":8}],6:[function(require,module,exports){
(function ($) {
    "use strict";
